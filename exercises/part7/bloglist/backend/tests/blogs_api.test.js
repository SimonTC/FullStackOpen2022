...
beforeAll(async () => {
  ...
  const loginResponse = await api.post(LOGIN_URL).send(loginInfo);
...
describe('adding a new blog', function () {
  ...
  test('automatically associates a user with the blog post', async () => {
    ...
    const creationResponse = await api.post(BASE_URL).send(newBlogPost);
  ...
  test('returns status code 400 if the title is missing ', async () => {
    ...
    await api.post(BASE_URL).send(blogPostWithoutTitle).expect(400);
  });
  ...
  test('returns status code 400 if the url is missing', async () => {
    ...
    await api.post(BASE_URL).send(blogPostWithoutUrl).expect(400);
  });
  ...
  test('fails if no token is given', async () => {
    ...
    await api.post(BASE_URL).send(newBlogPost).expect(401);
  });
...
describe('deleting a blog', function () {
  ...
  test('returns 204 if no blog with that id exist', async () => {
    ...
    await api.delete(`/api/blogs/${idOfNotExistingBlog}`).expect(204);
  });
...
describe('updating a blog', function () {
  test('is successful when data is valid', async () => {
    ...
    const blogsAfter = await helper.blogsInDb();
    expect(blogsAfter).toContainEqual(expect.objectContaining(updatedBlog));
  });
  ...
  test('works when only a single field is updated', async () => {
    ...
    expect(blogsAfter).toContainEqual(expect.objectContaining(expectedBlog));
  });
